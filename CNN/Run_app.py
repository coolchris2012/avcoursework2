...
# Change to AVCoursework1 directory if running directly
if __name__ == "__main__":
    script_dir = Path(__file__).resolve().parent  # CNN/
    project_root = script_dir.parent  # AVCoursework1/
...
def main():
    ...
    print(f"\nLoading dataset from {config.MFCC_DIR}...")
    train_dataset, test_dataset = MFCCDataset.from_split(
        data_dir=config.MFCC_DIR,
    ...
    class_mapping_path = config.RUN_FOLDER / "class_mappings.json"
    import json
    ...
    results_path = config.RUN_FOLDER / "test_summary.json"
    import json
    ...
    tester.plot_all_metrics(show=False)
